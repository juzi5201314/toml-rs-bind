...
export function stringify(v) {
    try {
        ...
        wasm.stringify(retptr, addBorrowedObject(v));
...
if (url.protocol.includes('file')) {
    ...
    if (Deno.build.os === 'windows') {
        ...
        file = Deno.realPathSync(file + '/../toml_rs_bind_bg.wasm')
    } else {
        file = file.substring(0, file.lastIndexOf('/') + 1) + 'toml_rs_bind_bg.wasm'
    }
    wasmCode = Deno.readFileSync(file)
} else if (url.protocol.includes('http')) {
    const wasm_url = import.meta.url.substring(0, import.meta.url.lastIndexOf('/') + 1) + 'toml_rs_bind_bg.wasm'
